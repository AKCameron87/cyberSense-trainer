...
import { CategoryScore, Badge, DIFFICULTY_CONFIGS, Difficulty } from '../../core/models/index';
...
export class ResultsComponent implements OnInit {
  ...
  mode:         string = '';
  score:        number = 0;  // multiplied score (actual points earned)
  baseScore:    number = 0;  // score before multiplier
  total:        number = 0;  // base total possible
  correct:      number = 0;
  streak:       number = 0;
  multiplier:   number = 1;
  bonusPoints:  number = 0;  // extra points from multiplier

  percentage:   number = 0;  // based on base score vs total
  ...
  async ngOnInit(): Promise<void> {
    ...
    // If baseScore wasn't passed (older sessions), estimate it
    if (!this.baseScore && this.multiplier > 1) {
      this.baseScore = Math.round(this.score / this.multiplier);
    } else if (!this.baseScore) {
      this.baseScore = this.score;
    }

    this.bonusPoints = Math.max(0, this.score - this.baseScore);

    // Percentage based on base score vs total — never exceeds 100%
  ...
  getDifficultyLabel(): string {
    const diff = this.progressService.getUserProgress().preferredDifficulty;
    return DIFFICULTY_CONFIGS.find(d => d.level === diff)?.label ?? '';
  }
