...
import { CategoryScore, Badge, UserProgress } from '../../core/models/index';
...
export class ResultsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.baseScore   = Number(params.get('baseScore'))  || 0;
    this.total       = Number(params.get('total'))      || 0;
    this.correct     = Number(params.get('correct'))    || 0;
    this.streak      = Number(params.get('streak'))     || 0;
    this.multiplier  = Number(params.get('multiplier')) || 1;

    if (!this.baseScore) {
      this.baseScore = this.multiplier > 1
        ? Math.round(this.score / this.multiplier)
        : this.score;
    }

    this.bonusPoints = Math.max(0, this.score - this.baseScore);
    this.percentage  = this.total > 0
      ? Math.min(100, Math.round((this.baseScore / this.total) * 100))
      : 0;
    this.grade       = this.getGrade(this.percentage);

    // Read progress once and use it for all derived data
  ...
  private async syncToFirebase(progress: UserProgress): Promise<void> {
    const user = this.authService.currentUser();
    if (!user) return;

    try {
      await this.firestoreService.saveProgress(user.uid, progress);
      await this.firestoreService.saveLeaderboardEntry(user.uid, {
        displayName: this.authService.userDisplayName,
        photoURL:    this.authService.userPhotoURL,
        totalPoints: progress.totalPoints,
        accuracy:    this.progressService.getAccuracyPercentage(),
  ...
  exportPDF(): void {
    ...
    const gradeBg = this.grade.bg
      .replace('bg-cyber-green',  '#00ff88')
      .replace('bg-cyber-accent', '#00d4ff')
      .replace('bg-cyber-yellow', '#ffd700')
      .replace('bg-cyber-red',    '#ff4d6d');

    const categoryHtml = progress.categoryScores.length ? `
      <div class="section">
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .grade-badge { display: inline-block; background: ${gradeBg}; color: #0a0e1a; padding: 6px 20px; border-radius: 20px; font-weight: bold; font-size: 18px; margin: 20px 0; }
...
<body>
  ...
  <div class="section">
    ...
    <div class="stats-grid">
      <div class="stat-box"><div class="stat-value" style="color:#00ff88">${progress.totalSessions}</div><div class="stat-label">Sessions</div></div>
      <div class="stat-box"><div class="stat-value">${progress.totalPoints}</div><div class="stat-label">Total Points</div></div>
      <div class="stat-box"><div class="stat-value" style="color:#00ff88">${progress.totalCorrect}</div><div class="stat-label">Correct Answers</div></div>
  ...
  ${categoryHtml}
  ${badgesHtml}
  <div class="footer">CyberSense Trainer · Security Awareness Training · ${date}</div>
</body>
</html>`;
